...
//! * `outputs.json` - the expected outputs from the task.
...
async fn run_test(test: &Path, result: AnalysisResult) -> Result<()> {
    ...
    if let Some(diagnostic) = diagnostics.iter().find(|d| d.severity() == Severity::Error) {
        return Err(diagnostic_to_error(result.document(), &path, diagnostic));
    }
    ...
    let (completed, outputs) = match evaluator
        ...
        .await
    {
        Ok((completed, outputs)) => {
            ...
            // Buffer the entire output before writing it out in case there are errors
        ...
        Err(e) => match e {
            ...
            EvaluationError::Other(e) => {
                return Err(e).with_context(|| format!("failed to evaluate document `{path}`"));
            }
        },
    };

    let stdout = fs::read_to_string(&completed.stdout).with_context(|| {
        format!(
            "failed to read task stdout file `{path}`",
            path = completed.stdout.display()
        )
    })?;
    let stderr = fs::read_to_string(&completed.stderr).with_context(|| {
        format!(
            "failed to read task stderr file `{path}`",
            path = completed.stderr.display()
        )
    })?;

    let outputs_path = test.join("outputs.json");
    compare_result(&outputs_path, &outputs)?;

    let stdout_path = test.join("stdout");
    compare_result(&stdout_path, &stdout)?;

    let stderr_path = test.join("stderr");
    compare_result(&stderr_path, &stderr)?;

    // Compare expected output files
    let mut had_files = false;
    let files_dir = test.join("files");
    for entry in WalkDir::new(&completed.work_dir) {
        let entry = entry.with_context(|| {
            format!(
                "failed to read directory `{path}`",
                path = completed.work_dir.display()
            )
        })?;
        ...
        let expected_path = files_dir.join(
            entry
                .path()
                .strip_prefix(&completed.work_dir)
    ...
    if files_dir.exists() {
        for entry in WalkDir::new(&files_dir) {
            ...
            let relative_path = entry
                ...
                .unwrap_or(entry.path());
            let expected_path = completed.work_dir.join(relative_path);
...
/// Creates an error from the given diagnostic.
fn diagnostic_to_error(document: &Document, path: &str, diagnostic: &Diagnostic) -> anyhow::Error {
    let source = document
        .root()
    ...
    anyhow!("document `{path}` contains at least one diagnostic error:\n{diagnostic}")
}
...
async fn main() {
    ...
    for test in &tests {
        ...
        match run_test(test, result).await {
            Ok(_) => {
