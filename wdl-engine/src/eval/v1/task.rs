...
/// A wrapper around a completed task that provides stdout and stderr values.
struct CompletedTaskValues {
    /// The completed task.
    task: CompletedTask,
...
impl CompletedTaskValues {
    /// Constructs a new execution outputs.
    fn new(task: CompletedTask) -> anyhow::Result<Self> {
        let stdout = PrimitiveValue::new_file(task.stdout.to_str().with_context(|| {
            format!(
                "path to stdout file `{path}` is not UTF-8",
                path = task.stdout.display()
            )
        })?)
        .into();
        let stderr = PrimitiveValue::new_file(task.stderr.to_str().with_context(|| {
            format!(
            ...
            .or_else(|| requirements.get("returnCodes"))
        {
            match return_codes {
                ...
                Value::Primitive(PrimitiveValue::Integer(ok)) => {
                    if self.task.status_code == i32::try_from(*ok).unwrap_or_default() {
                        error = false;
                    }
                }
                Value::Compound(CompoundValue::Array(codes)) => {
                    error = !codes.as_slice().iter().any(|v| {
                        v.as_integer()
                            .map(|i| i32::try_from(i).unwrap_or_default() == self.task.status_code)
                            .unwrap_or(false)
                    });
                }
        ...
        if error {
            bail!(
                "task process has terminated with status code {code}; see standard error output \
                 `{stderr}`",
                code = self.task.status_code,
                stderr = self.task.stderr.display(),
            );
        }
...
impl<'a> TaskEvaluator<'a> {
    ...
    ///
    /// Upon success, returns the outputs of the task.
    ///
    /// Upon error, returns a diagnostic.
    pub async fn evaluate(
        ...
        inputs: &TaskInputs,
        root: &Path,
    ) -> EvaluationResult<(CompletedTask, Outputs)> {
        ...
        match document.ast() {
            Ast::V1(ast) => {
                ...
                while current < nodes.len() {
                    match &graph[nodes[current]] {
                        ...
                        TaskGraphNode::Command(section) => {
                            ...
                            // Introduce the task variable into the output scope at this point;
                            // valid for both the command section and
                            // the outputs section
                ...
                let completed = CompletedTaskValues::new(
                    execution.spawn(command, &requirements, &hints)?.await?,
                )?;
                completed.handle_exit(&requirements)?;

                // Update the task return code
                if version >= SupportedVersion::V1(V1::Two) {
                    let task = scopes[TASK_SCOPE_INDEX]
                        ...
                        .as_task_mut()
                        .unwrap();
                    task.set_return_code(completed.task.status_code);
                }

                // Evaluate any remaining declarations
                for index in &nodes[current..] {
                    match &graph[*index] {
                        TaskGraphNode::Output(decl) => {
                            self.evaluate_output(document, &mut scopes, task, decl, &completed)?;
                        }
                        TaskGraphNode::Input(decl) => {
                            self.evaluate_input(
                                document,
                ...
                // Sort the outputs by declaration order
    ...
    fn evaluate_output(
        ...
        let mut evaluator = ExprEvaluator::new(
            TaskEvaluationContext::new(
                self.engine,
                document,
                &completed.task.work_dir,
                &completed.task.temp_dir,
                ScopeRef::new(scopes, TASK_SCOPE_INDEX),
            )
            .with_stdout(&completed.stdout)
            .with_stderr(&completed.stderr),
        );
        ...
        let value = value
            .coerce(self.engine.types(), ty) //, &completed.task.work_dir)
            .map_err(|e| {
                runtime_type_mismatch(
                    self.engine.types(),
                    e,
