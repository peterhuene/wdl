...
impl<'a> State<'a> {
    ...
    fn new(document: &'a Document, task: &'a Task, execution: Box<dyn TaskExecution>) -> Self {
        ...
        let scopes = [
            Scope::new(None),
            Scope::new(Some(ROOT_SCOPE_INDEX)),
            Scope::new(Some(OUTPUT_SCOPE_INDEX)),
        ];
...
impl<'a> TaskEvaluator<'a> {
    ...
    pub async fn evaluate(
        ...
        let mut state = State::new(
            document,
            task,
            self.engine.backend().create_execution(root)?,
        );
            ...
            match &graph[nodes[current]] {
                ...
                TaskGraphNode::Command(section) => {
                    ...
                    let constraints = state
                        .execution
                        .constraints(self.engine, &state.requirements, &state.hints)
                        .with_context(|| {
    ...
    fn evaluate_hints_section(
        ...
        for item in section.items() {
            ...
            let mut evaluator = ExprEvaluator::new(
                TaskEvaluationContext::new(self.engine, state, ROOT_SCOPE_INDEX).with_task(),
            );
    ...
    fn evaluate_output(
        ...
        let ty = self.engine.convert_ast_type_v1(state.document, &decl_ty)?;
        let mut evaluator = ExprEvaluator::new(
            TaskEvaluationContext::new(self.engine, state, TASK_SCOPE_INDEX)
