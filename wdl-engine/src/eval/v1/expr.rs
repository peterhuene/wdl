...
pub(crate) mod test {
    ...
    impl TestEnv {
        ...
        pub fn work_dir(&self) -> Option<&Url> {
            Some(&self.work_dir_url)
        }
    ...
    impl Default for TestEnv {
        fn default() -> Self {
            ...
            let work_dir_url = Url::from_file_path(
                absolute(work_dir.path()).expect("failed to make path absolute"),
            )
    ...
    impl Downloader for TestEnv {
        fn download<'a, 'b, 'c>(
            &'a self,
            url: &'b Url,
        ) -> BoxFuture<'c, Result<crate::http::Location, Arc<anyhow::Error>>>
        where
            'a: 'c,
            'b: 'c,
            Self: 'c,
