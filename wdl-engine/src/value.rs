...
pub trait Coercible {
    ...
    /// collection.
    fn coerce(&self, types: &Types, target: Type) -> Result<Value>;
}
...
impl Value {
    ...
    /// path value.
    pub(crate) fn join_paths(&mut self, path: &Path) {
        match self {
            Self::Primitive(v) => v.join_paths(path),
            Self::Compound(v) => v.join_paths(path),
            _ => {}
        }
    }
...
impl Coercible for Value {
    fn coerce(&self, types: &Types, target: Type) -> Result<Self> {
        ...
        match self {
            ...
            Self::Primitive(v) => v.coerce(types, target),
            Self::Compound(v) => v.coerce(types, target),
            Self::Task(_) => {
...
impl PrimitiveValue {
    ...
    /// Replaces any inner path values by joining the specified path with the
    /// path value.
    fn join_paths(&mut self, path: &Path) {
        match self {
            Self::File(p) | Self::Directory(p) => {
...
impl Coercible for PrimitiveValue {
    fn coerce(&self, types: &Types, target: Type) -> Result<Value> {
        if target.is_union() || target.is_none() || self.ty().type_eq(types, &target) {
            return Ok(Value::Primitive(self.clone()));
        }
...
impl CompoundValue {
    ...
    fn join_paths(&mut self, path: &Path) {
        match self {
            Self::Pair(pair) => {
                Arc::make_mut(&mut pair.left).join_paths(path);
                Arc::make_mut(&mut pair.right).join_paths(path);
            }
            Self::Array(array) => {
...
impl Coercible for CompoundValue {
    fn coerce(&self, types: &Types, target: Type) -> Result<Value> {
        if target.is_union() || target.is_none() || self.ty().type_eq(types, &target) {
            return Ok(Value::Compound(self.clone()));
        }
        ...
        if let Type::Compound(compound_ty) = target {
            match (self, types.type_definition(compound_ty.definition())) {
                ...
                (Self::Array(v), CompoundTypeDef::Array(array_ty)) => {
                    ...
                    return Ok(Value::Compound(Self::Array(Array::new(
                        types,
                        target,
                ...
                (Self::Map(v), CompoundTypeDef::Map(_)) => {
                    return Ok(Value::Compound(Self::Map(Map::new(
                        types,
                        target,
                        v.iter().map(|(k, v)| {
                ...
                (Self::Pair(v), CompoundTypeDef::Pair(_)) => {
                    return Ok(Value::Compound(Self::Pair(Pair::new(
                        types,
                        target,
                        v.left.as_ref().clone(),
                ...
                (Self::Map(v), CompoundTypeDef::Struct(struct_ty)) => {
                    ...
                    return Ok(Value::Compound(Self::Struct(Struct {
                        ty: target,
                ...
                (Self::Struct(Struct { members, .. }), CompoundTypeDef::Map(map_ty)) => {
                    ...
                    return Ok(Value::Compound(Self::Map(Map::new_unchecked(
                        target,
                        members
                            .iter()
                            .map(|(n, v)| {
                ...
                (Self::Object(v), CompoundTypeDef::Struct(_)) => {
                    return Ok(Value::Compound(Self::Struct(Struct::new(
                        types,
                        target,
        ...
        if let Type::Object = target {
            match self {
                // Map[String, Y] -> Object
                Self::Map(v) => {
                    return Ok(Value::Compound(Self::Object(
                        v.iter()
                            .map(|(k, v)| {
                                let k = k
                                    .as_ref()
                ...
                // Struct -> Object
                Self::Struct(v) => {
                    return Ok(Value::Compound(Self::Object(Object {
